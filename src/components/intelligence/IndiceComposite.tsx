...
import { AccentCard, CardContent } from "@/components/ui/card";
 ...
 *   - rose    si valeur < 0.4  (critique)
 *   - ambre   si 0.4 ≤ valeur < 0.7 (à surveiller)
 *   - émeraude si valeur ≥ 0.7  (sain)
 *
 * `unite` :
...
/** Seuil couleur pour une valeur 0-1 — palette Azure Bloom (rose/amber/emerald). */
function couleurPourValeur(v: number): string {
  if (v < 0.4) return "bg-rose-500";
  if (v < 0.7) return "bg-amber-500";
  return "bg-emerald-500";
}

/** Couleur du texte de la valeur — palette Azure Bloom. */
...
export function IndiceComposite({
  ...
  // Déterminer l'accent de carte selon la valeur
  const accent = valeurNulle || donneesInsuffisantes
    ? "amber"
    : (valeur as number) < 0.4
      ? "rose"
      : (valeur as number) < 0.7
        ? "amber"
        : "emerald";

  return (
    <AccentCard accent={accent} className="relative">
      <CardContent className="p-3 sm:p-4">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded-lg p-1.5 sm:p-2 text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1"
        ...
        {ouverte && (
          <div
            role="tooltip"
            className="absolute right-2 top-9 z-20 max-w-[15rem] sm:max-w-[20rem] rounded-xl border bg-popover p-2.5 text-xs text-popover-foreground shadow-md"
          >
            {description}
          </div>
        )}
        ...
        <div className="mt-2 flex items-baseline gap-1.5">
          {donneesInsuffisantes || valeurNulle ? (
            ...
            <span
              className={cn(
                "text-lg sm:text-xl font-bold tabular-nums font-data",
