...
const STATUS_CONFIG: Record<TenantStatus, { label: string; icon: typeof CheckCircle; color: string }> = {
  TRIAL:      { label: "Essai",      icon: Clock,         color: "text-yellow-500" },
  ACTIVE:     { label: "Actif",      icon: CheckCircle,   color: "text-green-500" },
  SUSPENDED:  { label: "Suspendu",   icon: AlertCircle,   color: "text-orange-500" },
  CANCELLED:  { label: "Résilié",    icon: XCircle,       color: "text-red-500" },
};
...
export function SuperAdminView() {
  ...
  const handleCreate = () => {
    startTransition(async () => {
      ...
      toast.success(`École "${form.name}" créée avec succès`);
      setShowForm(false);
      setForm(EMPTY_FORM);
      load();
    });
  ...
  const handleDelete = (id: string, name: string) => {
    if (!confirm(`Supprimer définitivement "${name}" et toutes ses données ? Cette action est irréversible.`)) return;
    startTransition(async () => {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Gestion des établissements EcolPro</p>
          </div>
        </div>
        <button
        ...
        >
          <Plus className="w-4 h-4" /> Créer une école
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { label: "Total écoles", value: stats.total, icon: School, color: "text-indigo-600" },
          { label: "Actives", value: stats.actifs, icon: CheckCircle, color: "text-green-600" },
          { label: "En essai", value: stats.trials, icon: Clock, color: "text-yellow-600" },
          { label: "Suspendues", value: stats.suspendus, icon: AlertCircle, color: "text-orange-600" },
          { label: "Total élèves", value: stats.totalEleves.toLocaleString("fr-FR"), icon: GraduationCap, color: "text-blue-600" },
        ].map((s) => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
          <BarChart3 className="w-4 h-4 text-indigo-500" /> Répartition par plan
        </h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">École</th>
                ...
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">Élèves</th>
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">Enseignants</th>
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">Utilisateurs</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Créée le</th>
                ...
                return (
                  <tr key={t.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <select
                        value={t.plan}
                        onChange={(e) => handlePlanChange(t.id, e.target.value as PlanType)}
                    ...
                    <td className="px-4 py-3 text-right font-medium text-gray-700">{t._count.eleves}</td>
                    <td className="px-4 py-3 text-right text-gray-500">{t._count.enseignants}</td>
                    <td className="px-4 py-3 text-right text-gray-500">{t._count.users}</td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h3 className="font-semibold text-gray-900">Créer une nouvelle école</h3>
            ...
            <div className="overflow-y-auto p-5 space-y-4">
              <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Informations de l'école</p>
              <div>
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Slug (sous-domaine) *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">Pays (ISO)</label>
              ...
              <p className="text-xs text-gray-500 font-medium uppercase tracking-wider pt-2">Compte administrateur</p>
              <div>
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Email admin *</label>
                <input type="email" value={form.adminEmail} onChange={(e) => f("adminEmail", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none" />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Mot de passe initial *</label>
