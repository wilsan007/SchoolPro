...
import { GenerateurRubrique } from "@/components/learnos/GenerateurRubrique";
 ...
 */
export default async function RubriquesEvaluationPage() {
  const [session, t] = await Promise.all([
    auth(),
    getTranslations("learnos.rubriqueEvaluation"),
  ]);
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 scrollbar-thin">
        <div className="mx-auto max-w-5xl">
          <GenerateurRubrique competences={competencesFormatees} />
        </div>
      </div>
    </div>
  );
