...
const TRUE = true;
const FALSE = false;
const NB_CHAPITRES = 5; // 5 chapitres par matière × niveau (cf. genFile10)
...
const DEMO_NOW = new Date(2026, 7, 16); // month is 0-indexed (7 = August)

// School year months for N-1 (2024-2025): sep(9)-dec(12) 2024, jan(1)-jul(7) 2025
const SCHOOL_MONTHS_N1 = [
  { y: 2024, m: 9 }, { y: 2024, m: 10 }, { y: 2024, m: 11 }, { y: 2024, m: 12 },
  { y: 2025, m: 1 }, { y: 2025, m: 2 }, { y: 2025, m: 3 }, { y: 2025, m: 4 },
  { y: 2025, m: 5 }, { y: 2025, m: 6 }, { y: 2025, m: 7 }
];

// School year months for N (2025-2026): sep(9)-dec(12) 2025, jan(1)-jul(7) 2026
const SCHOOL_MONTHS_N = [
...
// Default school months = N (2025-2026) for backward compatibility
const SCHOOL_MONTHS = SCHOOL_MONTHS_N;

// Resolve school months by cohort year: '2024' → N-1, '2025' → N
function schoolMonthsFor(anneeYear) {
  return anneeYear === '2024' ? SCHOOL_MONTHS_N1 : SCHOOL_MONTHS_N;
}

// Random date within the school year, formatted as "YYYY-MM-DD HH:MM:SS"
function schoolDate(hour = '00', minute = '00', anneeYear) {
  const months = anneeYear ? schoolMonthsFor(anneeYear) : SCHOOL_MONTHS;
...
// Décale une date "YYYY-MM-DD HH:MM:SS" de n jours, en conservant l'heure.
...
function schoolDayDate(hour = '00', minute = '00', anneeYear) {
...
function devoirDates(anneeYear) {
  const months = anneeYear ? schoolMonthsFor(anneeYear) : SCHOOL_MONTHS;
  const donneIdx = randInt(0, months.length - 3); // leave room for rendu
  const donneMo = months[donneIdx];
  const dateDonne = `${donneMo.y}-${String(donneMo.m).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00`;
  const renduIdx = Math.min(donneIdx + randInt(1, 3), months.length - 1);
  const renduMo = months[renduIdx];
...
//
// Le modèle est choisi via la variable MODELE_NIVEAUX ci-dessous.
// Les clés canoniques sont uniques pour éviter les collisions
// entre primaire et collège (ex: "3eme" collège vs "3emeA" primaire).

const MODELE_NIVEAUX = process.env.MODELE_NIVEAUX || 'ANNEES'; // 'ANNEES' ou 'FRANCAIS'

// Primaire — clés canoniques uniques
const PRIMAIRE_NIVEAUX_ANNEES = ['1A','2A','3A','4A','5A']; // 1ère→5ème année
...
function genFile04() {
  ...
  // Teachers
  const matieresList_TEST = ['MATH','FR','ANG','AR','HG','PC','SVT','EPS','TECH','ART','MUS','ISL','PHILO','SES'];
  for (const site of ['ambouli','arhiba']) {
    const siteId = `site-${site}`;
    for (let i = 1; i <= 20; i++) {
...
function genFile05() {
  ...
  for (const cls of classes) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Varied student status: ~3% TRANSFERE, ~2% ABANDONNE, rest ACTIF
      const statusRoll = rand();
      let eleveStatut = 'ACTIF';
      let dateSortie = null;
      let motifSortie = null;
      if (statusRoll < 0.03) {
        eleveStatut = 'TRANSFERE';
        dateSortie = `2025-${String(randInt(1,6)).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00`;
        motifSortie = pick(['Déménagement','Transfert vers autre établissement','Changement de ville','Rapprochement familial']);
      } else if (statusRoll < 0.05) {
        eleveStatut = 'ABANDONNE';
        dateSortie = `2025-${String(randInt(1,6)).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00`;
        motifSortie = pick(['Difficultés financières','Désintérêt scolaire','Contraintes familiales','Migration']);
      }

      // Special needs: ~5% of students
      const besoinsSpeciaux = rand() < 0.05 ? pick(['Dyslexie','TDAH','Trouble auditif','Trouble visuel','Difficultés motrices','Trouble du langage']) : null;

      // Scholarship: ~15% of students
      const numeroBoursier = rand() < 0.15 ? `BOURSE-${anneeYear}-${num}` : null;

      eleves.push([eleveId,'tenant-ambouli',cls.siteId,matricule,ln,fn,birthDate,'Djibouti','DJ',sexe,null,eleveStatut,cls.id,null,null,gs,null,besoinsSpeciaux,regime,null,contactNom,contactPhone,cls.annee,numeroBoursier,'2024-09-15 00:00:00',dateSortie,motifSortie,null,TS,TS,null]);
      ...
      // Recommandation d'orientation basée sur la moyenne et le niveau
      ...
      if (cls.niveau === 'Terminale' || cls.niveau === 'Terminale S' || cls.niveau === 'Terminale L') {
        ...
        else if (moyenne >= 10) recommandation = pick(['FILIERE_LITTERAIRE','TECHNIQUE']);
      ...
      if (isTerminale && isLastYear && moyenne >= 10) {
        ...
        const alStatut = weightedPick([['ETUDES_SUPERIEURES',5],['EN_EMPLOI',3],['RECHERCHE_EMPLOI',2],['INCONNU',1]]);
        const etab = alStatut === 'ETUDES_SUPERIEURES' ? pick(['Université de Djibouti','ENSI','EST','IUT','Université Paris-Saclay','Université de Tunis']) : (alStatut === 'EN_EMPLOI' ? pick(['Banque de Djibouti','Ministère','Port de Djibouti','Djibouti Télécom','EDD']) : null);
        const formation = alStatut === 'ETUDES_SUPERIEURES' ? pick(['Informatique','Génie civil','Médecine','Droit','Sciences économiques','Lettres']) : null;
        const linkedin = rand() < 0.35 ? `https://linkedin.com/in/${fn.toLowerCase()}-${ln.toLowerCase()}-${num}` : null;
        const accepteContact = rand() < 0.70;
        const alNotes = rand() < 0.30 ? `Ancien élève ${mention ?? 'passable'}, promotion ${cls.annee}` : null;
        alumni.push([alId,'tenant-ambouli',cls.siteId,eleveId,ln,fn,pEmail,pPhone,null,sexe,birthDate,cls.annee,cls.nom,mention,`DIP-${anneeYear}-${num}`,alStatut,etab,formation,'Djibouti','DJ',linkedin,accepteContact,alNotes,TS,TS]);
      }
...
function genFile06() {
  ...
  // Re-generate student IDs deterministically (same logic as genFile05)
  const classStudents = {};
  seed = 42; // reset
  // Re-run the same generation to get student IDs
  ...
  for (const cls of classes) {
    ...
    for (let i = 0; i < count; i++) {
      ec[site][anneeYear]++;
      const num = String(ec[site][anneeYear]).padStart(4, '0');
      const eleveId = `ele-${site}-${anneeYear}-${num}`;
      studs.push(eleveId);
    }
    classStudents[cls.id] = studs;
  }
  ...
  for (const cls of classes) {
    ...
    const periods = anneeYear === '2024'
      ...
      : ['per-y2025-t1-amb','per-y2025-t2-amb']; // skip T3 2025-2026 (not yet passed)
    ...
    for (const matCode of matiereCodes.slice(0, 8)) { // top 8 subjects
      ...
      for (let s = 0; s < nbSlots; s++) {
        ...
        const salleNom = pick(['Salle 101','Salle 102','Salle 103','Salle 201','Salle 202','Salle 203','Salle Info']);
        edt.push([`edt-${cls.id}-${matCode}-${s}`,'tenant-ambouli',cls.id,matId,ensId,jour,cr[0],cr[1],salleNom,cls.annee]);
      }
    }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const isPast = i < 7; // 7 past exams, 3 upcoming
    const dateDebut = isPast ? `2025-${String(randInt(10,12)).padStart(2,'0')}-01 00:00:00` : `2026-${String(randInt(1,5)).padStart(2,'0')}-01 00:00:00`;
    const dateFin = isPast ? `2025-${String(randInt(10,12)).padStart(2,'0')}-15 00:00:00` : `2026-${String(randInt(1,5)).padStart(2,'0')}-15 00:00:00`;
    const statut = isPast ? 'TERMINE' : 'PROGRAMME';
    const intitule = isPast
      ? `Examen ${niveauxExam[i % niveauxExam.length]} ${i < 5 ? 'Trimestre 1' : 'Trimestre 2'}`
      : `Examen Blanc ${niveauxExam[i % niveauxExam.length]}`;
    examens.push([exId,'tenant-ambouli',site,intitule,`Examen pour ${niveauxExam[i % niveauxExam.length]}`,statut,dateDebut,dateFin,TS,TS]);

    // 2-3 sessions per examen
    const nbSessions = randInt(2, 3);
    for (let s = 0; s < nbSessions; s++) {
      const sesId = `ses-${exId}-${s+1}`;
      const sesDate = isPast ? `2025-${String(randInt(10,12)).padStart(2,'0')}-${String(randInt(1,15)).padStart(2,'0')} 00:00:00` : `2026-${String(randInt(1,5)).padStart(2,'0')}-${String(randInt(1,15)).padStart(2,'0')} 00:00:00`;
  ...
  sql += batchInsert('examens', ['id','tenantId','siteId','intitule','description','statut','dateDebut','dateFin','createdAt','updatedAt'], examens) + '\n\n';
...
function genFile07() {
  ...
  for (const cls of classes) {
    ...
    const anneeYear = cls.annee === '2024-2025' ? '2024' : '2025';
    const isLycee = LYCEE_NIVEAUX.includes(cls.niveau);
    const montant = isLycee ? 212000 : 160000;
    const count = randInt(25, 30);
    for (let i = 0; i < count; i++) {
      ec[site][anneeYear]++;
      const num = String(ec[site][anneeYear]).padStart(4, '0');
      ...
      elevesList.push({ eleveId, parentId, site: cls.siteId, annee: cls.annee, isLycee, montant });
    }
  }
  ...
  for (const e of elevesList) {
    ...
    if (statut === 'EN_RETARD') {
      ...
      const envoyeePar = pick(['user-accountant-ambouli','user-accountant-arhiba','user-principal-ambouli']);
      for (let n = 1; n <= nbRelances; n++) {
        ...
        relances.push([`rel-${factId}-${n}`,'tenant-ambouli',factId,n,canal,`Relance niveau ${n} pour la facture F-${factIdx}`,envoyeePar,`2025-0${relMonth}-15 12:00:00`,TS]);
      }
    }
  }
  ...
  sql += batchInsert('relances', ['id','tenantId','factureId','niveau','canal','message','envoyeeParId','envoyeeLe','createdAt'], relances) + '\n\n';
...
function genFile08() {
  ...
  for (const cls of classes) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const num = String(ec[site][anneeYear]).padStart(4, '0');
      const eleveId = `ele-${site}-${anneeYear}-${num}`;
      allEleves.push({ id: eleveId, siteId: cls.siteId, site, classeId: cls.id, anneeYear });
    }
  }
  ...
  // Absences (20% of students, 1-8 each)
  const absEleves = allEleves.filter(() => rand() < 0.20);
  let absIdx = 0;
  for (const e of absEleves) {
    const nbAbs = randInt(1, 8);
    for (let i = 0; i < nbAbs; i++) {
      absIdx++;
      const isRetard = rand() < 0.30;
      const motif = pick(['MALADIE','INJUSTIFIE','FAMILIALE','TRANSPORT','AUTRE']);
      // Only generate absences on school days (Sunday-Thursday in Djibouti)
      const dateAbs = schoolDayDate('08', '00', e.anneeYear);
      const statut = isPast(dateAbs)
        ? weightedPick([['JUSTIFIEE',6],['INJUSTIFIEE',3],['EN_ATTENTE',1]])
        : 'EN_ATTENTE';
      // parentNotifie consistency: if TRUE, parentNotifieAt must be set; if FALSE, must be NULL
  ...
  for (const e of allEleves.filter(() => rand() < 0.10)) {
    ...
    passages.push([`pas-${pasIdx}`,'tenant-ambouli',e.siteId,e.id,schoolDate('10','00',e.anneeYear),motif,soin,suite,suite === 'retour_en_cours',randInt(15,60),e.site === 'ambouli' ? 'user-nurse-ambouli' : 'user-nurse-arhiba',null,TS,TS]);
  }
  ...
  for (const e of allEleves.filter(() => rand() < 0.08)) {
    incIdx++;
    const type = pick(incidentTypes);
    const gravite = randInt(1, 3);
    const dateInc = schoolDate('12', '00', e.anneeYear);
  ...
  for (let i = 0; i < dispEleves.length; i++) {
    ...
    dispenses.push([`disp-${i+1}`,'tenant-ambouli',e.id,`mat-EPS`,'2025-01-15 00:00:00','2026-07-15 00:00:00','Certificat médical',null,'ACCEPTEE',TS,TS]);
  }
  ...
  sql += batchInsert('dispenses_matiere', ['id','tenantId','eleveId','matiereId','dateDebut','dateFin','motif','certificatUrl','statut','createdAt','updatedAt'], dispenses) + '\n\n';
...
function splitAndWrite(filename, content) {
  ...
  console.log(`  [splitAndWrite] OUT_DIR=${OUT_DIR}, cleaning for ${filename}`);
...
function genFile09() {
      ...
      if (rand() < 0.40) {
        ...
        for (let a = 0; a < nb; a++) {
          ...
          const statut = isPast(absDate)
            ? weightedPick([['JUSTIFIEE',6],['INJUSTIFIEE',3],['EN_ATTENTE',1]])
            : 'EN_ATTENTE';
          absPers.push([`ap-${apIdx}`,'tenant-ambouli',ensId,absDate,type==='RETARD'?'08:00':null,type==='RETARD'?'08:30':null,type,statut,null,null,null,TS,TS]);
        }
      }
      // Congés (15%)
      if (rand() < 0.15) {
        ...
        if (statut === 'APPROUVE' || statut === 'TERMINE') {
          approuveParId = pick(['user-admin-amb','user-principal-ambouli','user-principal-coll-amb','user-principal-lycee-amb']);
  ...
  // Remplacements (35) — linked to varied classes, subjects, and teachers
  const rmplClasses = ['cls-ambouli-2025-6eme-A','cls-ambouli-2025-5eme-A','cls-ambouli-2025-4eme-A','cls-arhiba-2025-6eme-A','cls-arhiba-2025-5eme-A','cls-arhiba-2025-4eme-A','cls-ambouli-2025-3eme-A','cls-arhiba-2025-3eme-A'];
  const rmplMatieres = ['MATH','FR','ANG','HG','PC','SVT'];
  for (let i = 0; i < 35; i++) {
    rmIdx++;
    const site = i < 18 ? 'ambouli' : 'arhiba';
    const cls = rmplClasses[i % rmplClasses.length];
    const matCode = rmplMatieres[i % rmplMatieres.length];
    const rmplDate = schoolDate('08');
    const statut = isPast(rmplDate)
      ? weightedPick([['EFFECTUE',7],['VALIDE',2],['ANNULE',1]])
      : weightedPick([['PROPOSE',5],['VALIDE',4],['EN_ATTENTE',1]]);
    const motif = pick(['Maladie','Formation','Mission','Congé','Personnel']);
    remplacements.push([`rmpl-${i+1}`,'tenant-ambouli',`site-${site}`,null,cls,`mat-${matCode}`,`ens-${site}-${randInt(1,20)}`,`ens-${site}-${randInt(1,20)}`,rmplDate,'08:00','09:00',pick(['Salle 101','Salle 102','Salle 201','Salle 202']),statut,motif,null,'user-principal-coll-amb',TS,TS]);
  }

  // Devoirs (50) — répartis sur les deux cohortes N-1 (2024-2025) et N (2025-2026)
  for (let i = 0; i < 50; i++) {
    dvIdx++;
    const site = i < 25 ? 'ambouli' : 'arhiba';
    // Alterne entre cohortes: 25 devoirs N-1 (2024-2025), 25 devoirs N (2025-2026)
    const anneeYear = i % 2 === 0 ? '2024' : '2025';
    const cls = `cls-${site}-${anneeYear}-6eme-A`;
    const matCode = pick(['MATH','FR','ANG','HG','PC','SVT']);
    const { dateDonne, dateRendu } = devoirDates(anneeYear);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    conversations.push([convId,'tenant-ambouli',`Conversation ${i+1}`,isGroup,type,classeId,site,'user-admin-amb',i%2===0,false,false,TS,TS]);
  ...
  // DemandesLienParent (30) — self-service parent↔élève rattachement
  // Needed for analyserTauxValidationLien (ECHANTILLON_MIN=10)
  const demandesLien = [];
  for (let i = 0; i < 30; i++) {
    const site = i < 15 ? 'ambouli' : 'arhiba';
    const anneeYear = i < 10 ? '2024' : '2025';
    const parentNum = String((i % 50) + 1).padStart(4, '0');
    const eleveNum = String(((i + 30) % 50) + 1).padStart(4, '0');
    const parentId = `parent-${site}-${anneeYear}-${parentNum}`;
    const eleveId = `ele-${site}-${anneeYear}-${eleveNum}`;
    const matriculeSaisi = `${site.toUpperCase().slice(0,3)}-${anneeYear}-${eleveNum}`;
    const birthYear = anneeYear === '2024' ? 2013 : 2014;
    const dateNaissanceSaisie = `${birthYear}-${String(randInt(1,12)).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00`;
    const createdAt = `2025-${String(randInt(9,12)).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00`;
    const statut = weightedPick([['VALIDE',5],['EN_ATTENTE',3],['REFUSE',2]]);
    const traiteLe = statut === 'EN_ATTENTE' ? null : `2025-${String(randInt(10,12)).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00`;
    const traitePar = statut === 'EN_ATTENTE' ? null : 'user-admin-amb';
    const motifRefus = statut === 'REFUSE' ? pick(['Matricule ne correspond pas','Date de naissance incorrecte','Lien déjà existant','Documents manquants']) : null;
    demandesLien.push([`dlp-${i+1}`,'tenant-ambouli',parentId,eleveId,matriculeSaisi,dateNaissanceSaisie,statut,traitePar,traiteLe,motifRefus,createdAt,createdAt]);
  }
  ...
  sql += batchInsert('item_inventaire', ['id','tenantId','siteId','nom','description','reference','categorie','etat','quantite','quantiteMin','localisation','fournisseur','prixUnitaire','devise','dateAchat','dateGarantie','dateRevision','photoUrl','notes','createdAt','updatedAt'], inventaire) + '\n\n';
  ...
  const typesConseil = ['ADMINISTRATION','DISCIPLINE','PEDAGOGIQUE','AUTRE'];
  const conseilsData = [
  ...
  const adminUsers = ['user-admin-amb','user-principal-ambouli','user-principal-coll-amb','user-principal-lycee-amb','user-principal-arhiba'];
  conseilsData.forEach((c, i) => {
    ...
    // 2-3 réunions per conseil
    const nbReunions = randInt(2, 3);
    for (let r = 0; r < nbReunions; r++) {
      const rId = `reu-${cId}-${r+1}`;
      const isPastReunion = r < nbReunions - 1;
      const rDate = isPastReunion ? `2025-${String(randInt(10,12)).padStart(2,'0')}-01 00:00:00` : `2026-${String(randInt(1,3)).padStart(2,'0')}-01 00:00:00`;
      const rStatut = isPastReunion ? 'TERMINEE' : 'PLANIFIEE';
      reunions.push([rId,cId,`${c.nom} — Réunion ${r+1}`,rDate,`Salle de réunion ${i+1}`,`Ordre du jour point 1, point 2`,rStatut,isPastReunion ? `Compte-rendu de la réunion ${r+1}` : null,isPastReunion ? `'[]'::jsonb` : null,TS,TS]);
      // 1-2 résolutions per réunion passée
      if (isPastReunion) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // 2-3 objectifs per mentorat
    const nbObj = randInt(2, 3);
    for (let o = 0; o < nbObj; o++) {
      const objStatut = isPastMent ? pick(['ATTEINT','ATTEINT','NON_ATTEINT']) : pick(['EN_COURS','EN_COURS','ABANDONNE']);
    ...
    // 3-5 séances per mentorat
    const nbSeances = randInt(3, 5);
    for (let s = 0; s < nbSeances; s++) {
      const isPastSeance = isPastMent || s < nbSeances - 1;
      const sStatut = isPastSeance ? 'EFFECTUEE' : 'PLANIFIEE';
      const sDate = isPastSeance ? `2025-${String(randInt(1,6)).padStart(2,'0')}-${String(randInt(1,28)).padStart(2,'0')} 00:00:00` : `2026-${String(randInt(1,3)).padStart(2,'0')}-01 00:00:00`;
      seancesMent.push([`seance-${mId}-${s+1}`,mId,sDate,randInt(30,90),sStatut,isPastSeance ? `Compte-rendu séance ${s+1}` : null,pick(['Bureau','Salle de réunion','Visio']),TS,TS]);
    }
  }
  ...
  // déjà écoulées (1 → 29), avec un statut réaliste :
  //   - semaines passées : EFFECTUEE (85%), ANNULEE (10%), REPORTEE (5%)
  //   - semaine courante : PLANIFIEE
  //   - semaines futures : non générées
  sql += `-- JPP: Séances pédagogiques\nDELETE FROM "seances_competences" WHERE "seanceId" IN (SELECT id FROM "seances_pedagogiques" WHERE "tenantId"='tenant-ambouli');\nDELETE FROM "seances_pedagogiques" WHERE "tenantId"='tenant-ambouli';\n`;

  const seancesPeda = [], seancesComp = [];
  const jppMatieres = ['MATH','FR','ANG','HG','PC','SVT','EPS'];
  const jppClasses = [];
  for (const site of ['ambouli','arhiba']) {
    for (const niveau of ['6eme','5eme','4eme','3eme','2nde','1ere','Terminale']) {
      // Année N (2025-2026) — séances sur toute l'année
      jppClasses.push({ id: `cls-${site}-2025-${niveau}-A`, site: `site-${site}`, niveau, anneeYear: '2025', maxSem: 36 });
      // Année N+1 (2026-2027) — quelques séances en octobre (démo rentrée)
      jppClasses.push({ id: `cls-${site}-2026-${niveau}-A`, site: `site-${site}`, niveau, anneeYear: '2026', maxSem: 5 });
    }
  }
  let spIdx = 0, scIdx = 0;
...
function genFile10() {
  ...
  for (const matCode of matieresCodes) {
    ...
    for (const niveau of niveaux) {
      ...
      for (let c = 0; c < 2; c++) {
        ...
        // Planification chapitre (2025-2026)
        pcIdx++;
        planifsChap.push([`plch-${chId}`,'tenant-ambouli',null,'annee-2025-amb',chId,null,c*5+1,c*5+5,10,'TRAITE',`2025-01-15 00:00:00`,TS,TS,`2024-09-15 00:00:00`,c*5+1,c*5+5]);
      }
    }
  }
  ...
  // Within a chapter: comp-1 → comp-2 → comp-3 (progression logique)
  // Between chapters: chap-1-comp-3 → chap-2-comp-1 (passage au chapitre suivant)
  // Table de jointure Prisma: _CompetencePrerequis (A = prérequis, B = dépendant)
  const prerequisPairs = [];
  for (const matCode of matieresCodes) {
    for (const niveau of niveaux) {
      // Chain within chapter 1: 1→2→3
      prerequisPairs.push([`comp-${matCode}-${niveau}-1-1`, `comp-${matCode}-${niveau}-1-2`]);
      prerequisPairs.push([`comp-${matCode}-${niveau}-1-2`, `comp-${matCode}-${niveau}-1-3`]);
      // Chain within chapter 2: 1→2→3
      prerequisPairs.push([`comp-${matCode}-${niveau}-2-1`, `comp-${matCode}-${niveau}-2-2`]);
      prerequisPairs.push([`comp-${matCode}-${niveau}-2-2`, `comp-${matCode}-${niveau}-2-3`]);
      // Bridge: chapter 1 last → chapter 2 first
      prerequisPairs.push([`comp-${matCode}-${niveau}-1-3`, `comp-${matCode}-${niveau}-2-1`]);
    }
  }
  let prerequisSql = '';
  if (prerequisPairs.length > 0) {
    // Delete existing pairs for these competences first
    const allCompIds = [...new Set(prerequisPairs.flat())].map(id => `'${id}'`).join(',');
    prerequisSql = `DELETE FROM "_CompetencePrerequis" WHERE "A" IN (${allCompIds}) OR "B" IN (${allCompIds});\n`;
...
function genFile11() {
  ...
  // 100 students from 2024 cohort
  const eleveIds = [];
  for (let i = 1; i <= 100; i++) {
    eleveIds.push(`ele-ambouli-2024-${String(i).padStart(4,'0')}`);
  }

  // 6 competences from MATH-6eme (prerequisite chain: 1-1 → 1-2 → 1-3 → 2-1 → 2-2 → 2-3)
  const compChain = [
    'comp-MATH-6eme-1-1', 'comp-MATH-6eme-1-2', 'comp-MATH-6eme-1-3',
    'comp-MATH-6eme-2-1', 'comp-MATH-6eme-2-2', 'comp-MATH-6eme-2-3',
  ];

  // 3 assessment dates spread across the school year (for courbe d'oubli)
  // T1: October (initial learning), T2: January (after winter break — forgetting),
  // T3: May (after relearning — recovery)
  const evidenceDates = [
    { date: '2024-10-15 00:00:00', periode: 't1' },
  ...
  for (const eleveId of eleveIds) {
    // ── 1. Deterministic student profile ──
    const profileHash = (eleveId.charCodeAt(eleveId.length-1) + eleveId.charCodeAt(eleveId.length-2)) % 100;
    ...
    // ── 2. Evidences + Profiles per competence ──
    ...
    for (let ci = 0; ci < compChain.length; ci++) {
      ...
      if (ci > 0) {
        if (profileType === 'veryWeak') {
          // Very weak students fail even more on dependent competences (cascade effect)
          competenceBase = Math.max(0.03, masteryBase - 0.12 * ci);
        } else if (profileType === 'weak') {
          // Weak students struggle progressively on dependent competences
          competenceBase = Math.max(0.08, masteryBase - 0.08 * ci);
        } else if (profileType === 'average') {
          // Average students slightly struggle on later competences
          competenceBase = Math.max(0.25, masteryBase - 0.03 * ci);
        }
        // Good and excellent students maintain their level across the chain
      }

      // Generate 3 evidences with forgetting/relearning pattern
      // Pattern: E1 (initial) → E2 (after break: forgetting) → E3 (after relearning: recovery)
      const scores = [];
      // Forgetting rate depends on student profile (weak students forget more)
      const forgettingRate = profileType === 'veryWeak' ? 0.20 :
                             profileType === 'weak' ? 0.15 :
                             profileType === 'average' ? 0.10 :
                             profileType === 'good' ? 0.05 : 0.03;
      // Recovery rate depends on engagement (weaker students recover less)
      ...
      for (let e = 0; e < 3; e++) {
        ...
        let score;
        if (e === 0) {
          // T1: Initial assessment — base level
          score = competenceBase;
        } else if (e === 1) {
          // T2: After winter break — forgetting effect (score decreases)
          score = Math.max(0, competenceBase - forgettingRate);
        } else {
          // T3: After relearning — recovery (but may not fully reach initial level)
          score = Math.min(1, competenceBase - forgettingRate + recoveryRate);
        }
        score = Math.round(score * 100) / 100;
        scores.push(score);

        // Build evaluation/note IDs matching genFile06 patterns
        const evalId = `eval-cls-ambouli-2024-6eme-A-MATH-per-y2024-${evidenceDates[e].periode}-amb-0`;
        const noteId = `note-${evalId}-${eleveId}`;
        ...
        evidences.push([`ev-${evIdx}`,'tenant-ambouli',null,eleveId,cpId,matId,'note',
          `note-${evIdx}`,noteId,evalId,'DEVOIR',
          Math.round(score * 20 * 100) / 100, 20, score, 0.8, 1,
          errorType, errorConf, null,
          evidenceDates[e].date, evidenceDates[e].date]);
      }

      competenceScores[cpId] = scores;

      // Generate StudentLearningProfile from evidence scores (logically derived, not random)
      prIdx++;
      const avgScore = Math.round((scores.reduce((a,b) => a+b, 0) / scores.length) * 100) / 100;
      // Trend: compare first vs last evidence
      const trendDelta = scores[2] - scores[0];
      const trend = trendDelta > 0.03 ? 'hausse' :
                    trendDelta < -0.03 ? 'baisse' : 'stable';
      // Status derived from score thresholds (matching SEUILS_PAR_DEFAUT)
      let status = 'UNKNOWN';
      ...
      profils.push([`prof-${prIdx}`,'tenant-ambouli',null,eleveId,cpId,
        avgScore, 0.8, status, 3, evidenceDates[2].date, trend,
        null, null, null, TS, TS]);
    }

    // ── 3. Remediation workflow for weak/very weak students ──
    // LOGIQUE: Seuls les élèves faibles reçoivent des recommandations + interventions + plans
    // Les recommandations OBLIGATOIRE non résolues alimentent simulerRemediation
    // Les interventions COMPLETED avec before/after alimentent comparerTypesIntervention
    // Les plans TERMINE avec avant/après alimentent analyserEfficacitePlans + analyserEfficaciteEnseignants
    if (profileType === 'weak' || profileType === 'veryWeak') {
      const cpId = compChain[0]; // First competence (prerequisite — most critical)
      const baseMastery = competenceScores[cpId][0]; // Initial score before remediation

      // 3a. Recommandation — workflow: OBLIGATOIRE (unresolved for simulation) or RESOLU
      rcIdx++;
      const isResolved = profileHash % 3 === 0; // ~33% are resolved
      const recNiveau = profileType === 'veryWeak' ? 'CRITIQUE' : 'FRAGILE';
      const recStatut = isResolved ? 'RESOLU' : 'OBLIGATOIRE';
      recommandations.push([`rec-${rcIdx}`,'tenant-ambouli',null,eleveId,cpId,
        ...
        null, 0, null,
        isResolved ? 'user-admin-amb' : null,
        isResolved ? '2025-02-01 00:00:00' : null,
        isResolved ? '2025-04-15 00:00:00' : null,
        TS, TS, null]);

      // 3b. Intervention — COMPLETED with measured before/after
      // LOGIQUE: Different intervention types have different effectiveness
      // remediation: most effective (Δ ≈ 0.20-0.30)
      // prerequisite_review: moderate (Δ ≈ 0.15-0.25)
      // retest: least effective (Δ ≈ 0.08-0.15)
      ivIdx++;
      ...
      interventions.push([`iv-${ivIdx}`,'tenant-ambouli',null,eleveId,cpId,
        ...
        `ens-ambouli-${randInt(1,20)}`, // Different teachers for effectiveness ranking
        'COMPLETED',
        '2024-11-15 00:00:00',
        '2025-02-15 00:00:00',
        interventionDelta > 0.15 ? 'SUCCESS' : 'PARTIAL',
        baseMastery, masteryAfter,
        TRUE, 'user-admin-amb', '2024-11-10 00:00:00',
        TS, TS]);

      // 3c. Plan de progression — TERMINE with measured avant/après
      // LOGIQUE: Plans have different teachers for ranking, different deltas for effectiveness
      plIdx++;
      const planId = `plan-${plIdx}`;
      const planDelta = 0.12 + rand() * 0.18; // 0.12-0.30 improvement
      const masteryApres = Math.min(1, Math.round((baseMastery + planDelta) * 100) / 100);
      const responsableUserId = `ens-ambouli-${randInt(1,20)}`; // Varied teachers for ranking
      plans.push([planId,'tenant-ambouli',null,eleveId,'remediation','automatique','TERMINE',
        `Plan de remédiation pour ${cpId}`,
        'declenchement_plan_critiques',
        responsableUserId,
        'user-admin-amb', '2025-03-20 00:00:00',
        '2024-11-01 00:00:00', '2025-01-15 00:00:00', '2025-03-15 00:00:00',
        TRUE, baseMastery, masteryApres,
        masteryApres >= 0.55 ? 'SUCCES' : 'PARTIEL',
        TS, TS, 'mat-MATH', null]);

      // 3d. Etapes du plan — workflow logique: A_FAIRE → EN_COURS → VALIDEE
      // For TERMINE plans, most steps should be VALIDEE (correlation with success)
      for (let et = 0; et < 2; et++) {
        etIdx++;
        const etapeCpId = compChain[et];
        const echeance = et === 0 ? '2024-12-15 00:00:00' : '2025-02-15 00:00:00';
        // For completed plans: 80% VALIDEE, 15% FAIT, 5% A_FAIRE
        const etapeStatut = rand() < 0.80 ? 'VALIDEE' : (rand() < 0.75 ? 'FAIT' : 'A_FAIRE');
        etapes.push([`etape-${etIdx}`,planId,etapeCpId,et+1,
          `Étape ${et+1}: revoir ${etapeCpId}`,
          'enseignant', echeance, etapeStatut, null, null,
          etapeStatut === 'VALIDEE' ? echeance : null, TS, TS]);
      }
    }
  }

  // ── 4. Additional PROPOSE plans for current year (not yet started) ──
  // These represent upcoming remediation that hasn't begun yet
  for (let i = 0; i < 10; i++) {
    plIdx++;
  ...
  // Same logic as before: cover all evaluations from genFile06
  ...
  const evalPeriods = {
    ...
    '2025': ['per-y2025-t1-amb','per-y2025-t2-amb']
  };
  for (const site of ['ambouli','arhiba']) {
    for (const anneeYear of ['2024','2025']) {
      for (const niveau of ALL_NIVEAUX) {
...
function genFile12() {
  ...
  // LOGIQUE: ~60% générées par IA, ~40% validées (relueParId set) pour mesurerAdoptionIA
  ...
  for (const cpId of compIds) {
    for (let q = 0; q < 5; q++) {
      ...
      const isIa = rand() < 0.60; // 60% generated by IA
      const origine = isIa ? 'ia' : 'humain';
      // Of IA-generated questions, ~40% are reviewed by a teacher
      ...
      questions.push([`q-${qIdx}`,'tenant-ambouli',null,cpId,palier,
        `Question ${q+1} pour ${cpId}`,`Corrigé: réponse attendue`,1,
        origine, TRUE, TS, TS,
        pick(['CHOIX_UNIQUE','SAISIE_COURTE','SAISIE_LIBRE','APPARIEMENT']),
        `'{"propositions":["A","B","C","D"],"bonne":"A"}'::jsonb`,
        relueLe, relueParId]);
    }
  }
  ...
  // Etape IDs from genFile11: etape-1, etape-2, etc. (created for weak students with 2 per plan)
  // We know at least some etapes exist (weak students ~25% of 100 = ~25 students × 2 = ~50 etapes)
  // Link some feuilles to existing etapes (every 5th feuille)
  const knownEtapeIds = [];
  for (let i = 1; i <= 50; i++) {
    knownEtapeIds.push(`etape-${i}`);
  }
  for (const eleveId of eleveIds) {
    fIdx++;
...
function genFile13() {
  ...
  // Prédictions (100) — LOGIQUEMENT COHÉRENTES
  // La prédiction est émise AVANT le chapitre (probaReussite basée sur les prérequis)
  // La maîtrise réelle (masteryApres) est mesurée APRÈS le chapitre
  // predictionCorrecte = |proba - masteryApres| < 0.15 (la prédiction était proche de la réalité)
  // ecart = |proba - masteryApres| (distance entre prédiction et réalité)
  // Certaines prédictions sont correctes, d'autres non (pour que l'ISP puisse mesurer la précision)
  const eleveIds = [];
  for (let i = 1; i <= 50; i++) {
    eleveIds.push(`ele-ambouli-2024-${String(i).padStart(4,'0')}`);
  }
  for (const eleveId of eleveIds) {
    // Deterministic profile hash (same logic as genFile11)
    const profileHash = (eleveId.charCodeAt(eleveId.length-1) + eleveId.charCodeAt(eleveId.length-2)) % 100;
    let actualMastery;
    if (profileHash < 15) actualMastery = 0.82 + rand() * 0.15;
    else if (profileHash < 40) actualMastery = 0.62 + rand() * 0.18;
    else if (profileHash < 75) actualMastery = 0.42 + rand() * 0.18;
    else if (profileHash < 90) actualMastery = 0.22 + rand() * 0.16;
    else actualMastery = 0.08 + rand() * 0.12;
    actualMastery = Math.round(actualMastery * 100) / 100;
    ...
    for (let p = 0; p < 2; p++) {
      ...
      // La prédiction est basée sur la maîtrise des prérequis (masteryAvant)
      // masteryAvant ≈ actualMastery (l'élève arrive avec son niveau)
      const masteryAvant = Math.max(0, Math.round((actualMastery - rand() * 0.1) * 100) / 100);

      // probaReussite : la prédiction du moteur avant le chapitre
      // ~70% des prédictions sont raisonnablement proches, ~30% se trompent
      const isAccurate = rand() < 0.70;
      let proba;
      if (isAccurate) {
        // Prédiction proche de la réalité (±0.10)
        proba = Math.max(0.05, Math.min(0.95, actualMastery + (rand() * 0.20 - 0.10)));
      } else {
        // Prédiction qui se trompe (écart > 0.15)
        // Soit surestime un élève faible, soit sous-estime un élève fort
        const direction = actualMastery < 0.5 ? 1 : -1;
        proba = Math.max(0.05, Math.min(0.95, actualMastery + direction * (0.20 + rand() * 0.20)));
      }
      proba = Math.round(proba * 100) / 100;

      // masteryApres : la maîtrise réellement mesurée après le chapitre
      const masteryApres = actualMastery;

      // predictionCorrecte : la prédiction était-elle proche de la réalité ?
      const ecart = Math.abs(proba - masteryApres);
      const predictionCorrecte = ecart < 0.15;

      // difficultePredite : dérivée de la proba
      const diff = proba >= 0.7 ? 'FACILE' : (proba >= 0.5 ? 'MODERE' : (proba >= 0.3 ? 'DIFFICILE' : 'CRITIQUE'));

      // prerequisManquants : plus l'élève est faible, plus il a de prérequis manquants
      const prerequisManquants = actualMastery < 0.35 ? randInt(2, 4) : (actualMastery < 0.55 ? randInt(1, 2) : (actualMastery < 0.75 ? randInt(0, 1) : 0));

      // Dates: prédiction émise en septembre, vérifiée plus tard
  ...
  // Calibrations (7 niveaux)
  for (const niveau of niveaux) {
    calIdx++;
    calibrations.push([`cal-${calIdx}`,'tenant-ambouli',null,niveau,null,0.3,0.5,0.75,0.9,0.45,randInt(50,200),TRUE,randInt(2,8),TS,TS]);
  }
  ...
  // Alertes parent (50) — needed for analyserImpactAlertePaiement (ECHANTILLON_MIN=10)
  for (let i = 0; i < 50; i++) {
    alIdx++;
    const site = i < 25 ? 'site-ambouli' : 'site-arhiba';
    const siteName = site === 'site-ambouli' ? 'ambouli' : 'arhiba';
    const anneeYear = i < 25 ? '2024' : '2025';
    const num = String((i % 50) + 1).padStart(4, '0');
    const eleveId = `ele-${siteName}-${anneeYear}-${num}`;
    const parentId = `parent-${siteName}-${anneeYear}-${num}`;
  ...
  // Échanges parent (45) — needed for analyserQuestionsFrequentes (ECHANTILLON_MIN=10)
  // and analyserCorrelationEngagement (ECHANTILLON_MIN=10)
  const echangeQuestions = [
    { q: 'Comment va mon enfant en mathématiques ?', i: 'progression', r: 'Votre enfant progresse bien, moyenne actuelle: 12/20.' },
    { q: 'Mon enfant a-t-il des difficultés en français ?', i: 'difficultes', r: 'Des difficultés en lecture ont été détectées, un plan de soutien est en place.' },
    { q: 'Comment puis-je aider mon enfant à la maison ?', i: 'aider', r: 'Voici quelques exercices de révision à faire ensemble le soir.' },
    { q: 'Mon enfant était-il absent cette semaine ?', i: 'assiduite', r: 'Aucune absence signalée cette semaine.' },
    { q: 'Quel est le solde de ma facture ?', i: 'solde', r: 'Votre solde restant dû est de 45 000 DJF.' },
    { q: 'Quand sont les prochaines évaluations ?', i: 'progression', r: 'Les évaluations du 2ème trimestre ont lieu en janvier.' },
    { q: 'Mon enfant peut-il redoubler ?', i: 'difficultes', r: 'Une décision sera prise en conseil de classe en fin d\'année.' },
    { q: 'Y a-t-il des devoirs pour demain ?', i: 'progression', r: 'Oui, exercices page 45 en mathématiques.' },
    { q: 'Comment se comporte mon enfant en classe ?', i: 'assiduite', r: 'Comportement satisfaisant, participe activement.' },
    { q: 'Puis-je avoir un rendez-vous avec le professeur ?', i: 'aider', r: 'Un rendez-vous peut être fixé mercredi à 15h.' },
  ];
  for (let i = 0; i < 45; i++) {
    ecIdx++;
    const site = i < 23 ? 'site-ambouli' : 'site-arhiba';
    const siteName = site === 'site-ambouli' ? 'ambouli' : 'arhiba';
    const anneeYear = i < 25 ? '2024' : '2025';
    const parentNum = String((i % 50) + 1).padStart(4, '0');
    const parentId = `parent-${siteName}-${anneeYear}-${parentNum}`;
    const eleveId = `ele-${siteName}-${anneeYear}-${parentNum}`;
    const eq = echangeQuestions[i % echangeQuestions.length];
    const canal = pick(['whatsapp','sms','email','app']);
    echanges.push([`ech-${ecIdx}`,'tenant-ambouli',site,parentId,eleveId,canal,eq.q,eq.i,eq.r,null,TS]);
  }

  // Plans de leçon (15) — workflow complet: PROPOSE → AJUSTE → VALIDE | REJETE
  // LOGIQUE: mesurerAdoptionIA compte les plans validés vs proposés
  // Il faut une distribution réaliste des statuts avec métadonnées cohérentes
  const planStatuts = [
    { statut: 'PROPOSE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'PROPOSE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'PROPOSE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'AJUSTE', ajusteParId: 'user-prof-ambouli-1', ajusteLe: '2025-10-10 00:00:00', valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'AJUSTE', ajusteParId: 'user-prof-ambouli-2', ajusteLe: '2025-10-12 00:00:00', valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'AJUSTE', ajusteParId: 'user-prof-ambouli-3', ajusteLe: '2025-10-15 00:00:00', valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-1', ajusteLe: '2025-10-10 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-10-20 00:00:00', motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-2', ajusteLe: '2025-10-12 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-10-22 00:00:00', motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-3', ajusteLe: '2025-10-15 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-10-25 00:00:00', motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-4', ajusteLe: '2025-10-18 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-10-28 00:00:00', motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-5', ajusteLe: '2025-10-20 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-11-01 00:00:00', motifRejet: null },
    { statut: 'REJETE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: 'Objectifs trop ambitieux pour le niveau 6ème' },
    { statut: 'REJETE', ajusteParId: 'user-prof-ambouli-6', ajusteLe: '2025-10-08 00:00:00', valideParId: null, valideLe: null, motifRejet: 'Étapes mal structurées, manque de progression logique' },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-7', ajusteLe: '2025-11-01 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-11-10 00:00:00', motifRejet: null },
    { statut: 'PROPOSE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: null },
  ];
  for (let i = 0; i < planStatuts.length; i++) {
    ...
    plansLecon.push([`pl-${plIdx}`,'tenant-ambouli',null,cpId,'6eme',60,
      ...
      ps.statut, ps.ajusteParId, ps.ajusteLe, ps.valideParId, ps.valideLe, ps.motifRejet,
      'gpt-4', FALSE, TS, TS]);
  }
  ...
  const rubStatuts = [
    ...
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-3', ajusteLe: '2025-10-15 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-10-25 00:00:00', motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-4', ajusteLe: '2025-10-18 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-10-28 00:00:00', motifRejet: null },
    { statut: 'REJETE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: 'Barème non cohérent avec les coefficients de la matière' },
    { statut: 'REJETE', ajusteParId: 'user-prof-ambouli-5', ajusteLe: '2025-10-08 00:00:00', valideParId: null, valideLe: null, motifRejet: 'Critères trop vagues, manque de descripteurs par niveau' },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-6', ajusteLe: '2025-11-01 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-11-10 00:00:00', motifRejet: null },
    { statut: 'PROPOSE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'AJUSTE', ajusteParId: 'user-prof-ambouli-7', ajusteLe: '2025-11-05 00:00:00', valideParId: null, valideLe: null, motifRejet: null },
    { statut: 'VALIDE', ajusteParId: 'user-prof-ambouli-8', ajusteLe: '2025-11-08 00:00:00', valideParId: 'user-admin-amb', valideLe: '2025-11-15 00:00:00', motifRejet: null },
    { statut: 'PROPOSE', ajusteParId: null, ajusteLe: null, valideParId: null, valideLe: null, motifRejet: null },
  ];
  for (let i = 0; i < rubStatuts.length; i++) {
    ...
    rubriques.push([`rb-${rbIdx}`,'tenant-ambouli',null,cpId,'6eme',20,
      ...
      rs.statut, rs.ajusteParId, rs.ajusteLe, rs.valideParId, rs.valideLe, rs.motifRejet,
      'gpt-4', FALSE, TS, TS]);
  }
